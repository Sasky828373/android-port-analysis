...
static bool hasUsableEnginePresentSource(){return lastCompatFinalTransferDst.load(std::memory_order_acquire)!=nullptr||lastCompatDrawnRTV.load(std::memory_order_acquire)!=nullptr||lastCompatPrimaryRTV.load(std::memory_order_acquire)!=nullptr;}
