...
static bool compatGpuCopyResource(void*,void*); static bool compatGpuCopySubresource(void*,uint32_t,uint32_t,uint32_t,uint32_t,void*,uint32_t,const void*); static bool compatGpuResolveResource(void*,uint32_t,void*,uint32_t);
static void compatCtxCopySubresourceRegion(void*,void* dst,uint32_t ds,uint32_t x,uint32_t y,uint32_t z,void* src,uint32_t ss,const void* box){gtavdiag::checkpoint("compat-context-copy-subresource-region");if(!compatGpuCopySubresource(dst,ds,x,y,z,src,ss,box))gtavdiag::checkpoint("native-compat-gpu-copy-subresource-fallback");}
...
static void compatResolveBacking(void*,uint32_t,void*,uint32_t); static void compatCtxResolveSubresource(void*,void* dst,uint32_t ds,void* src,uint32_t ss,uint32_t){gtavdiag::checkpoint("compat-context-resolve-subresource");compatResolveBacking(dst,ds,src,ss);if(!compatGpuResolveResource(dst,ds,src,ss))gtavdiag::checkpoint("native-compat-gpu-resolve-fallback");}
...
static void rememberDrawnPrimaryRTV(void* ctx){std::lock_guard<std::mutex> l(mirrorMutex);auto it=mirrorStates.find(ctx);if(it==mirrorStates.end()||!it->second.rtvCount||!it->second.rtv[0])return;void* v=it->second.rtv[0];lastCompatDrawnRTV.store(v,std::memory_order_release);static std::atomic<uint32_t> dn{0};uint32_t n=dn.fetch_add(1,std::memory_order_relaxed);if(n<12||n%512==0){void* r=compatUnderlyingResource(v);auto* rr=r?compatResourceObject(r):nullptr;char d[192];snprintf(d,sizeof(d),"view=%p resource=%p compat=%u size=%ux%u fmt=%u",v,r,(rr&&rr->vtbl==gCompatTexture2DVtable)?1u:0u,(rr&&rr->descSize>=8)?((uint32_t*)rr->desc)[0]:0u,(rr&&rr->descSize>=8)?((uint32_t*)rr->desc)[1]:0u,(rr&&rr->descSize>=20)?((uint32_t*)rr->desc)[4]:0u);gtavdiag::checkpoint("native-last-drawn-rtv",d);}}
